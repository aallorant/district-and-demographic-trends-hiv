...
source(paste0(main_dir,'/fit_logistic.R'))
...
# for (indicator in indicators) {
  ...
  # for (region in c('Central Africa', 'Eastern Africa', 'Southern Africa', 'Western Africa')) {
    ...
    # for (sex. in unique(dt$sex)) {
      ...
      # Baseline formula
      formula_baseline <- Y ~ 1 + f(res_idx, constr = TRUE, 
      ...
      # adding covariate-year interactions
      formula3 <- Y ~ 1 + f(res_idx, constr = TRUE, 
                            group = period_idx, control.group = list(model = "iid"),
                            hyper = list(prec = list(prior = "pc.prec", param = c(2.5, 0.01))))+
        f(age_idx, model = "iid", constr = TRUE, 
          group = period_idx, control.group = list(model = "iid"),
      ...
      # Same models but with spatial random effects instead of iid RE for area
      formula4 <- update(formula2,
                         . ~ . -f(area_idx, model = "iid", constr = TRUE,
                                  group = period_idx, control.group = list(model = "iid"),
                                  hyper = list(prec = list(prior = "pc.prec", 
                                                           param = c(2.5, 0.01)))) +
                           f(area_idx, model = "besag", graph = adjM, group = period_idx,
                             control.group = list(model = "iid"),
                             scale.model = TRUE, hyper = list(prec = list(prior = "pc.prec", 
                                                                          param = c(2.5, 0.01)))))
      
      formula5 <- update(formula3,
                         . ~ . -f(area_idx, model = "iid", constr = TRUE,
                                  group = period_idx, control.group = list(model = "iid"),
                                  hyper = list(prec = list(prior = "pc.prec", 
                                                           param = c(2.5, 0.01)))) +
      ...
      #' Which model has the lowest waic?
      # keep model with covariates
      res_df_best <- filter(res_df, model == paste0(models[[which.min(ic_df$waic)]]))
      res_fit_best <- res_fit[[which.min(ic_df$waic)]]
